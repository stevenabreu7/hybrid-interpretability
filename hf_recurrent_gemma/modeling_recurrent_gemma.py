...
def dejavu_intervention(attn_weights, attn_output, k: int, metric = "l2", do_prefill = False, attn_mask=None, head_mask_recorder=None):
    ...
    # NOTE: if prefill should be sparsified is specified when activating sparsity
    out_len = attn_weights.shape[-2]
    if do_prefill or out_len == 1:

        # Calculate metricon attention outputs, across Lg dim
...
class RecurrentGemmaSdpaAttention(nn.Module):
    ...
    def __init__(self, config: RecurrentGemmaConfig):
        super().__init__()
        self.config = config
        self.topk_heads = config.topk_heads #DZ
        self.sparsity_metric = config.sparsity_metric
        self.sparsity_prefill = config.sparsity_prefill
    ...
    def forward(
        ...
        attn_bias = torch.zeros(bsz, 1, L, S, dtype=query_states.dtype, device=query_states.device)
        ...
        self.attn_pattern_recorder(attn_weights)
        ...
        if self.topk_heads is not None:
            ...
            logger.debug(f"did topk dejavu with k={self.topk_heads}")
...
class RecurrentGemmaForCausalLM(RecurrentGemmaPreTrainedModel, GenerationMixin):
    ...
    def set_sparse_attributes(self, k: Optional[int] = None, metric: Optional[str] = "l2", prefill: Optional[bool] = False):
        # update config
        ...
        for layer in self.model.layers:
            if isinstance(layer.temporal_block, RecurrentGemmaSdpaAttention):
                layer.temporal_block.topk_heads = k
                layer.temporal_block.sparsity_metric = metric
                layer.temporal_block.sparsity_prefill = prefill
    ...
    def enable_sparsification(self, k: int = 2, metric = "l2", prefill: bool = False):
        ...
        logger.info(f"enabled sparsification with {k=}, {metric=}, {prefill=}")

    def disable_sparsification(self):
        """disable attention head sparsification"""
        self.set_topk_heads(None)
        self.config.sparsity_metric = None
        self.config.sparsity_prefill = None
        logger.info("disabled sparsification")
